  ...
  static async cacheEssentialData(userId: string) {
    try {
      const [properties, conversations, notifications] = await Promise.all([
        ...
        supabase.from('conversations').select('*').or(`landlord_id.eq.${userId},tenant_id.eq.${userId}`),
        supabase.from('notifications').select('*').eq('user_id', userId).limit(50)
      ]);
  ...
  private static async executeAction(action: any) {
    switch (action.type) {
      ...
      case 'update_property':
        return supabase.from('properties').update(action.data).eq('id', action.id);
