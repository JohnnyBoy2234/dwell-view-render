...
export default function Auth() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const { error, isNewUser } = await signUp(signUpData.email, signUpData.password, signUpData.role as 'tenant' | 'landlord' | 'agent');
  ...
  const handleGoogleSignIn = async (role: 'tenant' | 'landlord' | 'agent') => {
  ...
  const handleResendVerificationEmail = async () => {
    try {
      const { error } = await signUp(pendingEmail, signUpData.password, signUpData.role as 'tenant' | 'landlord' | 'agent');
      if (error) {
        throw error;
      }
    } catch (error: any) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 flex items-center justify-center p-4">
        <div className="w-full max-w-md space-y-6">
          <div className="text-center">
            <h1 className="text-3xl font-bold tracking-tight text-foreground">RentLekker</h1>
                  ...
                  <form onSubmit={handleSignUp} className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <RadioGroup 
                        ...
                        onValueChange={(value) => setSignUpData({...signUpData, role: value as 'tenant' | 'landlord' | 'agent'})}
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor="agent">Agent - Listing for clients</Label>
                        </div>
                      </RadioGroup>
                    </div>
