-- Create lease_agreements table to replace old system
...
-- Prevent updates to completed/immutable leases except for signature data
CREATE POLICY "Immutable lease agreements cannot be modified"
  ON public.lease_agreements
  FOR UPDATE
  USING (
    NOT immutable OR 
    (immutable AND status = 'pending_signatures' AND (
      (auth.uid() = landlord_id AND landlord_signed_at IS NULL) OR
      (auth.uid() = tenant_id AND tenant_signed_at IS NULL)
    ))
  );

-- Create lease_signatures table for detailed signature audit
...
CREATE INDEX idx_lease_signatures_agreement ON public.lease_signatures(lease_agreement_id);
...
CREATE POLICY "Users can view signatures for their lease agreements"
  ...
  USING (
    EXISTS (
      ...
      WHERE la.id = lease_agreement_id
