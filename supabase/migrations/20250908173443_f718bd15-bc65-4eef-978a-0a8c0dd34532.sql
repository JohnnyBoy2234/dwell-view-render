...
-- 1.2 Create viewing slots table (one slot per proposal; attach to a conversation)
create table if not exists public.viewing_slots (
  id uuid primary key default gen_random_uuid(),
  conversation_id uuid not null references public.conversations(id) on delete cascade, -- DM conversation
...
create index on public.viewing_slots(conversation_id);
