...
async function generateProfessionalLeasePDF(leasePack: any): Promise<Uint8Array> {
  ...
  const scheduleData = [
    ...
    [`Monthly Rent`, `R ${leasePack.core.monthlyRentZAR.toLocaleString()} • Due Day: ${leasePack.core.rentDueDay} • Method: ${leasePack.core.paymentMethod}`],
    [`Deposit`, `R ${leasePack.core.depositZAR.toLocaleString()} (Held in: ${leasePack.core.depositHeldIn}) • Refund: within ${leasePack.core.depositRefundDays} days`],
    ...
    [`Minor Repair Limit (Tenant)`, `R ${leasePack.core.maintenanceMinorRepairLimitZAR.toLocaleString()}`],
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ 
      ...
      pdf_hash: pdfHash,
      page_count: 0, // Will be calculated in production
      filename 
    }), { 
      status: 200, 
